...
#ifndef _DRIVER_C_SPI_SLAVE_H_
#define _DRIVER_C_SPI_SLAVE_H_
...
typedef struct {
    ...
    int queue_size;                 ///< Transaction queue size. This sets how many transactions can be 'in the air' (queued using c_spi_slave_queue_trans but not yet finished using c_spi_slave_get_trans_result) at the same time
...
struct spi_slave_transaction_t {
    ...
    void *rx_buffer;                /**< Pointer to receive buffer, or NULL for no MISO phase.
                                     ...
                                     */
    void *rx_buffer2;               ///Secondary buffer for circular receiving
 ...
 * not to access this memory until ``c_spi_slave_queue_trans`` is called to hand ownership back to the application.
 *
 ...
 *         - ESP_OK                on success
 */
esp_err_t c_spi_slave_queue_trans(spi_host_device_t host, const spi_slave_transaction_t *trans_desc, TickType_t ticks_to_wait);
 ...
 * c_spi_slave_queue_trans) has succesfully completed. It will then return the description of the
 * completed transaction so software can inspect the result and e.g. free the memory or
 * re-use the buffers.
 *
 * It is mandatory to eventually use this function for any transaction queued by ``c_spi_slave_queue_trans``.
 *
 ...
 *         - ESP_OK                on success
 */
esp_err_t c_spi_slave_get_trans_result(spi_host_device_t host, spi_slave_transaction_t **trans_desc, TickType_t ticks_to_wait);


/**
 * @brief Do a SPI transaction
 *
 * Essentially does the same as c_spi_slave_queue_trans followed by c_spi_slave_get_trans_result. Do
 * not use this when there is still a transaction queued that hasn't been finalized
 * using c_spi_slave_get_trans_result.
 *
 * @param host SPI peripheral to that is acting as a slave
 ...
 *         - ESP_OK                on success
 */
esp_err_t c_spi_slave_start_transmit(
